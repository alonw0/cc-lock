    ...
    if (lock.status === "locked") {
        ...
        if (lock.expiresAt) {
            ...
            if (expiry) {
                ...
                if (remaining > 0) {
                    parts.push(`<span class="countdown">Unlocks in ${formatDuration(remaining)}</span>`);
        ...
        if (!lock.hardLock && lock.bypassAttempts > 0) {
            parts.push(`<span class="bypass-warn">Bypass attempts: ${lock.bypassAttempts}</span>`);
        }
    }
        ...
        if (lock.graceExpiresAt) {
            ...
            if (expiry) {
                const remaining = (expiry.getTime() - now) / 1000;
                if (remaining > 0) {
                    parts.push(`<span class="countdown">Re-locks in ${formatDuration(remaining)}</span>`);
                }
            }
        }
    }
...
function renderSettings(data) {
    const toggle = document.getElementById("login-item-toggle");
    if (!toggle)
        return;
...
function renderStatus(data) {
    ...
    const labelMap = {
        ...
        locked: lock.hardLock ? "Hard locked — no bypass" : "Claude Code is locked",
...
document.getElementById("start-daemon-btn")?.addEventListener("click", () => {
    const btn = document.getElementById("start-daemon-btn");
